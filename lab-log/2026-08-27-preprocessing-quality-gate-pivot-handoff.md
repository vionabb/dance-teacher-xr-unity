...
**Phase 1 signals script exists and ran once against a legacy-adapted corpus (summary below); it's mid-rewrite to drop that adapter in favor of a real, freshly-extracted canonical corpus, blocked right now only on the local machine having enough free RAM to finish an unattended GPU extraction run.** Not yet committed on this branch — see "Immediate next action" at the bottom before doing anything else.

**What changed since the first Phase 1 pass** (prompted by Viona: re-extract the whole corpus through the canonical pipeline instead of adapting the participant studies' pre-canonical format in memory, and do it on GPU):
- Confirmed via `gh issue view` on `google-ai-edge/mediapipe` that the combined Holistic Landmarker Task (pose+hands+face) has a real, upstream, 2+-year-open, unresolved crash on any empty-detection sub-packet (mediapipe#5181), reproducing the exact crash the 2026-08-08 lab-log entry hit. The *standalone* Pose Landmarker Task does not share that bug and does support a GPU (Metal) delegate on macOS — validated directly against both a synthetic blank frame and real corpus frames, and its landmark output matches the existing CPU `Holistic` output within ~0.01-0.015 normalized units (expected model-to-model variance). Per-frame GPU throughput measured at ~8ms vs. ~89ms/frame CPU (~11x).
- Renamed the holistic file/directory naming for clarity now that a third modality exists: `.holisticdata.raw.csv` → `.holistic.raw.csv` (and `holisticdata/` → `holistic/` directories), in `pose-processing/dance_teacher_pose/schema.py` and every consumer (7 files total — see git history on this branch for the full list). Migrated the 9 existing on-disk reference holistic files to match; no participant-study holistic data exists yet to migrate.
- Added a new `pose3d` modality (`PoseDataType.pose3d`, suffix `.pose3d.raw.csv`, own directory alongside `pose2d`/`holistic`) carrying world-space (metric, hip-relative) pose landmarks from the same GPU Pose Landmarker pass — Viona wants this available for future 3D-motion investigation even though nothing consumes it yet.
- Added `dance_teacher_pose.extraction.extract_pose_landmarker_video()` (new, GPU-capable, pose-only) alongside the existing, untouched `extract_holistic_video()` (CPU, still the only source of hand data — per Viona, the frontend doesn't actually use hand/face features yet; Holistic was chosen originally in anticipation of maybe needing them).
- Added `motion_extraction/corpus_videos.py` (shared corpus-video enumeration, factored out so the extraction driver and the signals script can't drift on what "the corpus" means) and `motion_extraction/scripts/extract_pose_landmarker_corpus.py` (the corpus-wide driver: resumable, per-clip error isolation, writes `pose2d`+`pose3d` for every clip missing them).
- Validated end-to-end on real clips (small `--max-files` samples): correct output schema, correct directory placement per corpus, ~1.5-1.9s/participant-clip, ~4-5s/reference-clip — full-corpus estimate ~50-60 minutes total (vs. ~3-5 hours the original CPU-only plan would have taken).

**Blocked on**: a full-corpus run needs the machine to hold enough free RAM for an uninterrupted GPU pass. Two attempts today were killed by macOS's jetsam/OOM manager (`SIGKILL`, exit 137) with the system down to ~91MB free out of 16GB (confirmed via `vm_stat` and `log show --predicate 'eventMessage CONTAINS "jetsam"'`, which also showed other apps being killed around the same time) — this is genuine system-wide memory pressure from everything else running (Signal, VS Code, several Electron apps, Firefox, Spotify, ChatGPT all resident), not a leak in the extraction script itself (its own RSS oscillated in a bounded ~1.4-2.4GB range across every sample, never climbing toward the kill). Viona chose to close some apps and ask for a retry rather than have this session auto-retry-loop through the contention. The extraction script is resumable (skips clips whose pose2d+pose3d already exist), so a retry — whenever it comes — will pick up wherever the last attempt left off.

## Immediate next action

1. Confirm with Viona that enough RAM is free (or just retry — the extraction script is resumable either way).
2. From `motion-pipeline/`: `.venv/bin/python -m motion_extraction.scripts.extract_pose_landmarker_corpus --output-root temp/experiments/<new-name>`. If it dies again (exit 137 or otherwise), just re-run the same command with the same `--output-root` — clips already extracted are skipped, nothing is lost. It only writes `results.csv`/`run_provenance.json` on a full clean completion, so a partial/killed attempt leaves no misleading "done" marker.
3. Once it completes, re-run `compute_automatic_quality_signals.py`'s full sweep (adapter already removed from this branch's working tree — verify `git diff` before committing) and spot-check that the new canonical-pose2d-derived numbers for a few clips are in the same ballpark as the first (legacy-adapted) sweep's numbers for those same clips, as a sanity check that re-extraction didn't silently change the corpus's character.
4. Commit this branch's pending changes (schema rename, pose3d addition, new extraction driver, adapter removal) as one or more scoped commits, and update this Status section again before ending the session.
5. Only then does Phase 1's original exit criterion apply: review the resulting CSV with Viona before building any Stage 1/2 annotation-tool UI (Phases 2-3, still design-only).
...
### Full-sweep results, first pass (1,808 clips enumerated, 1,765 with pose data: 9 reference + 675 study1 + 1,081 study2 -- via the now-removed legacy adapter; numbers below are provisional until re-run against the canonically re-extracted corpus per "Immediate next action")
