...
6. Restart the annotation server (see "How to resume") to pick up the skeleton-overlay landing, then open `error-marking-009` and confirm its existing marks and the new overlay/per-landmark list look right together before annotating further on it.

## How to resume

**Server**: check first (`lsof -iTCP:8765 -sTCP:LISTEN`) before starting a new one — as of 2026-09-03 it's running locally on port 8765, pointed at `temp/experiments/20260828-quality-triage-batch-v1` / `data/human-annotations/quality-triage/annotations.sqlite3`. **It needs a restart to pick up the skeleton-overlay landing** (commit `1a54110`) since the manifest and `server.py` are only read at process start; stop it (Ctrl-C, or `kill` its PID) and restart with the exact same command below to resume — autosave/resume means nothing is lost:
```
